...
final class DefaultAnnotationExtractor implements AnnotationExtractor {
    ...
    @Override
    public List<RenderableAnnotation> extractAnnotations(List<Feature> features, Translation i18n) {
        return features
                .stream()
                .map(Feature::getComponents)
                .flatMap(Collection::stream)
                .map(featureComponent -> new RenderableAnnotation(getMessage(featureComponent, i18n), getTextSelection(featureComponent)))
                .toList();
    }
